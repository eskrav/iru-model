...
* $P_{S_1}(u|s;\alpha,C) \propto P(u;C)\exp(\alpha\log P_{L_0}(s|u))$
* $P_{L_1}(s|u) \propto P_{S_1}(u|s;\alpha,C) \cdot P(s)$
...
* $P_{S_1}(u|s,h;\alpha,C) \propto P(u;C)\exp(\alpha\log P_{L_0}(s|u,h))$
* $P_{L_1}(s,h|u) \propto P_{S_1}(u|s,h;\alpha,C) \cdot P(s|h) \cdot P(h)$
...
* $P_{S_1}(u_i|s,h;\alpha,C) \propto P(u;C)\exp(\alpha \sum\limits_{u_r}P(u_r|u_i)\log P_{L_0}(s|u_r,h))$
* $P_{L_1}(s,h|u_r) \propto P(s|h) \cdot P(h) \cdot \sum\limits_{u_i}P_{S_1}(u_i|s,h;\alpha,C)P(u_r|u_i)$
...
As noted above, $\alpha$ here does not change the effect of utterance costs ([see here for more info](https://michael-franke.github.io/probLang/chapters/app-03-costs.html)).
